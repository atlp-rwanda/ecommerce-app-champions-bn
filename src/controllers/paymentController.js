...
class PaymentController {
  static async paymentCheckout(req, res) {
    ...
    try {
      ...
      const amountpaid = cart.total;
      const session = await stripe.checkout.sessions.create({
        line_items: orderItems,
        mode: "payment",
        success_url: `${process.env.REDIRECT_URL}/paymentsuccess?token=${token}&&paymentId={CHECKOUT_SESSION_ID}&&amount=${amountpaid}`
      });
      res
        .status(200)
        .json({
      ...
      if (session.payment_status === "paid") {
        ...
        cart.products.map(async (element) => {
          ...
          const sales = await Sale.create({
            ...
            VendorId: product.dataValues.VendorId,
            Quantity: element.quantity
          });
          await sales.save();
          const owner = await Vendor.findOne({
            where: { id: sales.dataValues.VendorId },
            include: [{ model: User }]
          });
          ...
          await Product.update(
            { quantity: product.dataValues.quantity - element.quantity },
            { where: { productId: element.productId } }
          );
        });
